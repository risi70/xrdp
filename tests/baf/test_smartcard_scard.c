 ...
 *
 * The scard_function_*_return() parsers decode [MS-RDPESC] IOCTL responses.
 * They are static, so we #include the
 ...
 *     init_stream() buffer checked by AddressSanitizer.
 *
 * Build/run under -fsanitize=address,undefined for additional boundary
 * validation (see tests/baf/Makefile.am, guarded by XRDP_SMARTCARD).
 *
 * Vectors cover positive decodes, truncated headers, invalid lengths and
 * unsupported counts.
 */
 ...
 * from libcommon is used; only the wire send is faked. */
...
/* ================================================================== */
/* Response boundary vectors. Invalid input must fail closed.          */
/* ================================================================== */
static void test_response_boundaries(void)
{
    ...
    printf("[response boundary handling]\n");

    /* Truncated EstablishContext header (10 of 32 bytes). */
    new_client(&id);
    bb_reset(&b);
    bb_zeros(&b, 10);
    in_from_bb(&in, &b);
    CHECK(scard_function_establish_context_return((void *)(tintptr)id, &in, b.n, 0) == 1,
          "truncated establish_context header fails closed");

    /* Context length outside the supported range. */
    new_client(&id);
    bb_reset(&b);
    bb_zeros(&b, 28);
    bb_u32(&b, 0xFFFFFFFF);
    ...
    in_from_bb(&in, &b);
    CHECK(scard_function_establish_context_return((void *)(tintptr)id, &in, b.n, 0) == 1,
          "negative context_bytes rejected");

    /* Oversized context length. */
    new_client(&id);
    bb_reset(&b);
    bb_zeros(&b, 28);
    bb_u32(&b, 64);
    ...
    in_from_bb(&in, &b);
    CHECK(scard_function_establish_context_return((void *)(tintptr)id, &in, b.n, 0) == 1,
          "oversized context_bytes rejected");

    /* Transmit payload length exceeds the available input. */
    {
        struct pcsc_transmit *pt = (struct pcsc_transmit *)
                                   g_malloc(sizeof(struct pcsc_transmit), 1);
        new_client(&id);
        ...
        CHECK(scard_function_transmit_return((void *)pt, &in, b.n, 0) == 1,
              "transmit oversized cbRecvLength fails closed");
    }
    ...
    /* Transmit metadata length exceeds the available input. */
    {
        struct pcsc_transmit *pt = (struct pcsc_transmit *)
                                   g_malloc(sizeof(struct pcsc_transmit), 1);
        new_client(&id);
        ...
        in_from_bb(&in, &b);
        CHECK(scard_function_transmit_return((void *)pt, &in, b.n, 0) == 1,
              "transmit oversized PCI extra_bytes fails closed");
    }

    /* Card handle length outside the supported range. */
    {
        ...
        CHECK(scard_function_connect_return((void *)(tintptr)id, &in, b.n, 0) == 1,
              "connect oversized card_bytes rejected");
    }

    /* Negative card handle length. */
    {
        struct pcsc_uds_client *c = new_client(&id);
        char ctxbuf[] = "ctx";
        c->connect_context = uds_client_add_context(c, ctxbuf, 3);
        ...
        in_from_bb(&in, &b);
        CHECK(scard_function_connect_return((void *)(tintptr)id, &in, b.n, 0) == 1,
              "connect negative card_bytes rejected");
    }

    /* Reader count outside the supported range. */
    new_client(&id);
    bb_reset(&b);
    bb_zeros(&b, 28);
    bb_u32(&b, 0x7fffffff);
    ...
    CHECK(scard_function_get_status_change_return((void *)(tintptr)id, &in, b.n, 0) == 1,
          "get_status_change unsupported cReaders rejected");

    /* Reader array shorter than the declared count. */
    new_client(&id);
    bb_reset(&b);
    bb_zeros(&b, 28);
    bb_u32(&b, 10);
    ...
    in_from_bb(&in, &b);
    CHECK(scard_function_get_status_change_return((void *)(tintptr)id, &in, b.n, 0) == 1,
          "get_status_change short reader array fails closed");

    /* Truncated control header. */
    new_client(&id);
    bb_reset(&b);
    bb_zeros(&b, 8);
    in_from_bb(&in, &b);
    CHECK(scard_function_control_return((void *)(tintptr)id, &in, b.n, 0) == 1,
          "control truncated header fails closed");

    /* Control payload length exceeds the available input. */
    new_client(&id);
    bb_reset(&b);
    bb_zeros(&b, 28);
    bb_u32(&b, 0x40000000);
    in_from_bb(&in, &b);
    CHECK(scard_function_control_return((void *)(tintptr)id, &in, b.n, 0) == 1,
          "control oversized cbRecvLength fails closed");

    /* Clamp ATR lengths to the fixed field size. */
    {
        ...
        CHECK(scard_function_status_return((void *)ps, &in, b.n, 0) == 0,
              "status oversized dwAtrLen clamped");
    }

    /* Normalize a high-bit state value before the table lookup. */
    {
        struct pcsc_status *ps = (struct pcsc_status *)
                                 g_malloc(sizeof(struct pcsc_status), 1);
        new_client(&id);
        ...
        in_from_bb(&in, &b);
        CHECK(scard_function_status_return((void *)ps, &in, b.n, 0) == 0,
              "status high-bit dwState normalized");
    }

    /* Truncated list_readers header. */
    {
        ...
        CHECK(scard_function_list_readers_return((void *)pl, &in, b.n, 0) == 1,
              "list_readers truncated header fails closed");
    }

    /* Every status==0 parser with a completely empty stream. */
    {
        char empty[1];
        ...
        CHECK(scard_function_transmit_return((void *)pt, &in, 0, 0) == 1,
              "empty stream transmit fails closed");
        ...
        CHECK(scard_function_status_return((void *)ps, &in, 0, 0) == 1,
              "empty stream status fails closed");
        ...
        CHECK(scard_function_list_readers_return((void *)pl, &in, 0, 0) == 1,
              "empty stream list_readers fails closed");
    }
...
test_request_parsers(void)
{
    ...
    /* Exercise request parsers with short input under ASan. */
    new_client(&id);
    bb_reset(&b);
    bb_zeros(&b, 3);
    in_from_bb(&in, &b);
    ...
    CHECK(1, "process truncated TRANSMIT/CONNECT/STATUS input handled");
}
...
/* Send a complete framed PC/SC message: [size][command][body]. */
static int
...
test_socket_transport(void)
{
    ...
    /* An unsupported declared size is rejected before reading a body. */
